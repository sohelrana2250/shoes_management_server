...
async function run() {
  try {
    ...
    const productDB = client.db("shoesmanagement");
    const shoesCollection = productDB.collection("allSoehes");
    const userCollection = productDB.collection("user");
    // product
    ...
    app.get("/users/alluserdata", verifyToken, async (req, res) => {
    ...
    app.patch("/user/profile/:id", verifyToken, async (req, res) => {
    ...
    app.get("/dashboard/items", verifyToken, async (req, res) => {
      const totalUserCount = await userCollection.estimatedDocumentCount();
      const totalProductCount = await shoesCollection.estimatedDocumentCount();
      const allshoes = await shoesCollection.find({}).toArray();
      res.send({ totalUserCount, totalProductCount, allshoes });
    });
